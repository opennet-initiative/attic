...
# now done in /usr/sbin/check_ongateway.sh
...
#~ while [ -n "$(cat /proc/net/ip_conntrack | grep 'dst=192.168.0.251 .*dport=1600 src')" ]; do
	#~ $DEBUG && logger -t opennet_ugw_up.sh "waiting for registered connections to suspend"
	#~ sleep 5;
#~ done
